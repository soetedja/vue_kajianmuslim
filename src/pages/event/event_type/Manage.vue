  ...
  <div class="contents">
    <div class="white-box">
      <div class="content-title">
        <div class="row">
          <div class="col-xs-5">
            <h2>{{ $resources("List") }} <b> {{ $resources("EventType") }} </b> </h2>
            <p>{{ $resources("SubTitleManageEventTypes") }}</p>
          </div>
    ...
    <uiv-modal v-model="deleteModal" :title="$resources('DeleteConfirmation')">
      <span><b>{{ $resources("Msg_DeleteConfirmation", $resources("EventType"), name) }}</b></span>
      <div slot="footer">
...
export default {
  ...
  methods: {
     ...
     */
    editAction (user) {
      this.$router.push({ name: 'EditEventType', params: { id: user.id } })
    },
    ...
    gotoAdd () {
      this.$router.push({ name: 'AddEventType' })
    },
