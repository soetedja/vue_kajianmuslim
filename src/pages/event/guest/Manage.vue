  ...
  <div class="contents">
    <div class="white-box">
      <div class="content-title">
        <div class="row">
          <div class="col-xs-5">
            <h2>{{ $resources("List") }} <b> {{ $resources("Guest") }} </b> </h2>
    ...
    <uiv-modal v-model="deleteModal" :title="$resources('DeleteConfirmation')">
      <span><b>{{ $resources("Msg_DeleteConfirmation", $resources("Guest"), name) }}</b></span>
      <div slot="footer">
...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'alias', data: 'alias', title: this.$resources('Alias') },
        { name: 'short_description', data: 'short_description', title: this.$resources('ShortDescription') },
        { name: 'province_id', data: 'province_id', title: this.$resources('Province') },
        { name: 'phone_number', data: 'phone_number', title: this.$resources('PhoneNumber') },
        { name: 'instagram', data: 'instagram', title: this.$resources('Instagram') },
  ...
  methods: {
     ...
     */
    editAction (user) {
      this.$router.push({ name: 'EditGuest', params: { id: user.id } })
    },
