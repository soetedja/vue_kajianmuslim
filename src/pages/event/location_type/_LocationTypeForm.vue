<template>
  <form @submit.prevent="saveLocationType">
    ...
    <uiv-modal v-model="deleteModal" :title="$resources('DeleteConfirmation')">
      <span><b>{{ $resources("Msg_DeleteConfirmation", $resources("LocationType"), model.name) }}</b></span>
      <div slot="footer">
...
export default {
  ...
  data () {
    return {
      vmodel: {},
      statusesByEntity: this.$store.getters.getStatusByEntity('LOCATION_TYPE'),
      deleteModal: false
    }
  },
  created () {
