  ...
  <div class="contents">
    <div class="white-box">
      <div class="content-title">
        <div class="row">
          <div class="col-xs-5">
            <h2>{{ $resources("List") }} <b>{{ $resources("Event") }} </b></h2>
    ...
    <uiv-modal v-model="deleteModal" :title="$resources('DeleteConfirmation')">
      <span><b>{{ $resources("Msg_DeleteConfirmation", $resources("Event"), title) }}</b></span>
      <div slot="footer">
...
export default {
  data () {
    return {
      ...
      perPage: 10,
      columns: [
        // { name: 'id', data: 'id', title: '#', width: '5%' },
        // { name: 'poster_url', data: 'poster_url', title: this.$resources('Poster') },
        {
          name: 'poster_url',
          data: 'poster_url',
          render: function (data, type, row, meta) {
            return '<img src="' + row.poster_url.thumbnail + '" width="50">'
          }
        },
        { name: 'title', data: 'title', title: this.$resources('Title') },
        { name: 'description', data: 'description', title: this.$resources('Description'), width: '20%' },
        { name: 'start_time', data: 'start_time', title: this.$resources('Start') },
        { name: 'finish_time', data: 'finish_time', title: this.$resources('Finish') },
        { name: 'status_id', data: 'status_id', title: this.$resources('Status') },
        {
          name: '__actions',
  ...
  methods: {
     ...
     */
    editAction (user) {
      this.$router.push({ name: 'EditEvent', params: { id: user.id } })
    },
