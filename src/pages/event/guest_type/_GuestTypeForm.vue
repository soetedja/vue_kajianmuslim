<template>
  <form @submit.prevent="saveGuestType">
    ...
    <uiv-modal v-model="deleteModal" :title="$resources('DeleteConfirmation')">
      <span><b>{{ $resources("Msg_DeleteConfirmation", $resources("GuestType"), model.name) }}</b></span>
      <div slot="footer">
...
export default {
  ...
  data () {
    return {
      vmodel: {},
      statusesByEntity: this.$store.getters.getStatusByEntity('GUEST_TYPE'),
      deleteModal: false
    }
  },
  created () {
