  ...
  <div class="contents">
    <div class="white-box">
      <div class="content-title">
        <div class="row">
          <div class="col-xs-5">
            <h2>{{ $resources("Manage") }} <b>{{ $resources("Status") }}</b></h2>
            <p>{{ $resources("SubTitleManageStatuses") }}</p>
          </div>
...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'name', data: 'name', title: 'Name', width: '20%' },
        { name: 'label', data: 'label', title: 'Label', width: '20%' },
  ...
  methods: {
     ...
     */
    editAction (user) {
      this.$router.push({ name: 'EditStatus', params: { id: user.id } })
    },
    ...
    gotoAdd () {
      this.$router.push({ name: 'AddStatus' })
    },
