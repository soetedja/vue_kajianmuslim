  ...
  <div class="contents">
    <div class="white-box row">
      <div class="col-xs-4">
        ...
        <hr />
        <a @click="getEventsInBounds()" class="button btn btn-primary">
          <i class="fa fa-chevron-circle-left fa-fw"></i>
          <span>Get Events</span>
        </a>
        <a @click="getUserLocation()" class="button btn btn-primary">
          <i class="fa fa-chevron-circle-left fa-fw"></i>
          <span>Get UserLoc</span>
        </a>
        ...
        <button @click="pgPath = originalPlPath">Reset Polygon to a simple polygon</button><br> Path: {{pgPath}}
        <br>
        <h1>Circle</h1>
        Visible: <input type="checkbox" number v-model="displayCircle"><br> {{circleBounds}}
        <br>
        <h1>Rectangle</h1>
        Visible: <input type="checkbox" number v-model="displayRectangle"><br> {{rectangleBounds}}
        <br>
        <h1>PlaceInput</h1>
        ...
        modal 1 : <input type="checkbox" number v-model="ifw"><br> modal 2: <input type="checkbox" number v-model="ifw2"> <input type="text" v-model="ifw2text">
        <h1>Markers</h1>
      ...
      <div class="col-xs-8">
        ...
        :center="center" 
        :zoom="zoom" :map-type-id="mapType" :options="{styles: mapStyles, scrollwheel: scrollwheel}" @rightclick="mapRclicked" @drag="drag++" @click="mapClickedCount++" class="map-panel" @zoom_changed="update('zoom', $event)" @center_changed="update('reportedCenter', $event)"
...
export default {
  ...
  methods: {
    ...
    getEventsInBounds () {
      var south = this.mapBounds.getSouthWest().lat()
      var west = this.mapBounds.getSouthWest().lng()
      var north = this.mapBounds.getNorthEast().lat()
      var east = this.mapBounds.getNorthEast().lng()
      console.log('south:', south, 'west:', west, 'north:', north, 'east:', east)
      var paramsInbound = { 'south': south, 'west': west, 'north': north, 'east': east }
      axios.post(`http://localhost/Kajianmuslim.id/api/v1/event/getInBounds`, paramsInbound)
      .then(response => {
