  ...
  <form @submit.prevent="saveStatus">
    <div class="col-lg-6 col-md-5 col-sm-12 col-xs-12">
      <div class="sidebar-block">
        ...
        <input-text v-model="vmodel.group_name" fieldName="group" fieldText="Group" required />
        <input-text v-model="vmodel.name" fieldName="name" fieldText="Name" required />
        <input-text v-model="vmodel.route" fieldName="route" fieldText="Route" required />
        <input-text v-model="vmodel.method" fieldName="method" fieldText="Method" required />
    ...
    <uiv-modal v-model="deleteModal" :title="$resources('DeleteConfirmation')">
      <span><b>{{ $resources("Msg_DeleteConfirmation", $resources("Privilege"), model.name) }}</b></span>
      <div slot="footer">
...
export default {
  ...
  data () {
    return {
      deleteModal: false,
      vmodel: {},
      statusesByEntity: this.$store.getters.getStatusByEntity('PRIVILEGE')
    }
  },
  created () {
