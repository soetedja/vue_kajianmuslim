  ...
  <div class="contents">
    <div class="white-box">
      <div class="content-title">
        <div class="row">
          <div class="col-xs-5">
            <h2>{{ $resources("List") }} <b> {{ $resources("Privilege") }} </b> </h2>
            <p>{{ $resources("SubTitleManagePrivileges") }}</p>
          </div>
    ...
    <uiv-modal v-model="deleteModal" :title="$resources('DeleteConfirmation')">
      <span><b>{{ $resources("Msg_DeleteConfirmation", $resources("ParticipantType"), name) }}</b></span>
      <div slot="footer">
...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'group_name', data: 'group_name', title: this.$resources('Group') },
        { name: 'name', data: 'name', title: this.$resources('Name') },
        { name: 'route', data: 'route', title: this.$resources('Route') },
        { name: 'method', data: 'method', title: this.$resources('Method') },
  ...
  methods: {
     ...
     */
    editAction (user) {
      this.$router.push({ name: 'EditPrivilege', params: { id: user.id } })
    },
