  ...
  <div class="contents">
    <div class="white-box">
      <div class="content-title">
        <div class="row">
          <div class="col-xs-5">
            <h2>{{ $resources("Manage") }} <b>{{ $resources("User") }}</b></h2>
            <p>{{ $resources("SubTitleManageUsers") }}</p>
          </div>
          <div class="add-button col-xs-7 text-right">
            <button @click="reloadTable()" class="btn btn-primary"><i class="fa fa-refresh"></i> <span> {{ $resources("Reload") }}</span></button>
            <button @click="gotoAdd()" class="btn btn-primary"><i class="fa fa-edit"></i> <span> {{ $resources("Add") }}</span></button>
          </div>
        </div>
      </div>
  ...
  data () {
    return {
      ...
      columns: [
        ...
        {
          title: this.$resources('Picture'),
          class: 'text-center',
          name: 'picture_url',
          data: 'picture_url',
          render: function (data, type, row, meta) {
            return '<img src="' + row.picture_url.thumbnail + '" width="30" class="img-circle">'
          }
        },
        { name: 'email', data: 'email', title: this.$resources('Email'), width: '20%' },
        { name: 'username', data: 'username', title: this.$resources('Username'), width: '20%' },
        { name: 'first_name', data: 'first_name', title: this.$resources('FirstName'), width: '20%' },
        { name: 'last_name', data: 'last_name', title: this.$resources('LastName') },
        {
          name: '__actions',
          title: this.$resources('Action'),
          width: '10%',
